...
public enum RenderUtils
{
	...
	{
		Camera camera = NsylClient.MC.getBlockEntityRenderDispatcher().camera;
		if(camera == null)
			return Vec3.ZERO;
	...
	{
		return NsylClient.MC.renderBuffers().bufferSource();
	}
		...
		int color, boolean depthTest)
	{
		...
		RenderType layer = NsylRenderLayers.getLines(depthTest);
		...
		int color, boolean depthTest)
	{
		...
		MultiBufferSource.BufferSource vcp = getVCP();
		RenderType layer = NsylRenderLayers.getLineStrip(depthTest);
		VertexConsumer buffer = vcp.getBuffer(layer);
		...
		boolean depthTest)
	{
		...
		MultiBufferSource.BufferSource vcp = getVCP();
		RenderType layer = NsylRenderLayers.getQuads(depthTest);
		VertexConsumer buffer = vcp.getBuffer(layer);
		...
		float x2, float y2, int color)
	{
		...
		context.drawSpecial(consumers -> {
			VertexConsumer buffer =
				consumers.getBuffer(NsylRenderLayers.ONE_PIXEL_LINES);
		...
		float x2, float y2, int color)
	{
		...
		context.drawSpecial(consumers -> {
			VertexConsumer buffer =
				consumers.getBuffer(NsylRenderLayers.ONE_PIXEL_LINE_STRIP);
			buffer.addVertex(matrix, x1, y1, 1).setColor(color);
	...
	public static void drawBoxShadow2D(GuiGraphics context, int x1, int y1,
		int x2, int y2)
	{
		float[] acColor = NsylClient.INSTANCE.getGui().getAcColor();
