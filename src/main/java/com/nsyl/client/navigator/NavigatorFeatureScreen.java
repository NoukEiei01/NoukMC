 ...
 */
package com.nsyl.client.navigator;
...
import com.nsyl.client.keybinds.Keybind;
...
public final class NavigatorFeatureScreen extends NavigatorScreen
{
		...
		if(hasPrimaryAction)
		{
			primaryButton = Button.builder(
				...
				b -> {
					...
					NsylClient.INSTANCE.getNavigator()
		...
		if(!possibleKeybinds.isEmpty())
		{
			...
			ButtonData addKeybindButton =
					...
					area.y + getStringHeight(text) - 7, 12, 8, "+", 0x00ff00)
				{
					...
					{
						// add keybind
						NsylClient.MC.setScreen(new NavigatorNewKeybindScreen(
			...
			for(Keybind keybind : NsylClient.INSTANCE.getKeybinds()
				.getAllKeybinds())
			{
	...
	{
		// popups
		if(NsylClient.INSTANCE.getGui().handleNavigatorPopupClick(x, y,
			button))
			return;
		...
		// component settings
		NsylClient.INSTANCE.getGui().handleNavigatorMouseClick(
	...
	{
		NsylClient.INSTANCE.getGui().handleMouseRelease(x, y, button);
	}
	...
	{
		window.close();
		NsylClient.INSTANCE.getGui().handleMouseClick(Integer.MIN_VALUE,
			Integer.MIN_VALUE, 0);
	}
