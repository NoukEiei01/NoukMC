...
public class NavigatorRemoveKeybindScreen extends NavigatorScreen
{
	...
	{
		String oldCommands =
			NsylClient.INSTANCE.getKeybinds().getCommands(selectedKey);
		if(oldCommands == null)
			return;
		...
		if(commandsList.isEmpty())
			NsylClient.INSTANCE.getKeybinds().remove(selectedKey);
		else
		{
			String newCommands = String.join("\u00a7", commandsList)
				.replace(";", "\u00a7\u00a7").replace("\u00a7", ";");
			NsylClient.INSTANCE.getKeybinds().add(selectedKey, newCommands);
		}
	...
	protected void onMouseClick(double x, double y, int button)
	{
		...
		{
			NsylClient.MC.setScreen(parent);
			return;
		}
